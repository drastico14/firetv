...
description: "Orchestrazione in parallelo e composizione tipografica di lavori accademici lunghi (40-100+ pagine): tesi, dissertazioni, report. Da usare quando si tratta di parallelizzare la lavorazione su più agenti (schedatura del materiale, stesura simultanea dei capitoli), comporre il documento in LaTeX, compilarlo, verificare rinvii interni ed etichette duplicate, controllare che le citazioni non siano inventate. Attivare per richieste come \"scrivi i capitoli in parallelo\", \"componi la tesi in LaTeX\", \"compila il PDF\", \"controlla i rinvii\", \"verifica la bibliografia\", \"audit delle note\", \"write my thesis/FYP report\", \"compile the LaTeX\", \"check citations\". Per la tesi di specializzazione in contabilità pubblica di questo repository il metodo, la struttura e lo stile restano governati dalla skill tesi-contabilita-pubblica: questa fornisce solo l'orchestrazione e la composizione, secondo references/adattamento-tesi-giuridica.md."
version: 1.0.0
author: Haoyang Pang
license: MIT
...
> **Leggere prima `references/adattamento-tesi-giuridica.md`.**
>
> Questa skill è importata da un progetto di terzi (v. `ORIGIN.md`) ed è scritta per report STEM in inglese:
> repository di ricerca, esperimenti, figure, citazioni autore-anno. La tesi di questo repository è un lavoro
> giuridico italiano su fonti documentali. Il documento di adattamento dice quali parti della pipeline valgono
> qui, quali vanno scartate e perché. In caso di contrasto prevale `tesi-contabilita-pubblica`; le indicazioni
> del relatore prevalgono su entrambe.
>
> In breve: si prendono l'architettura a ondate, il cancello di fine ondata, il divieto di inventare citazioni e
> `cross_ref_audit.py`; si scartano gli agenti che analizzano codice e dati sperimentali, la pipeline delle
> figure, i modelli di capitolo Ch1–Ch6 e il template `university-thesis/`; `citation_checker.py` si usa solo
> sulla dottrina indicizzata (§4 del documento di adattamento).
...
### Aggiunte di questo repository

| Documento | Contenuto |
|----------|----------|
| [ORIGIN.md](ORIGIN.md) | provenienza della skill, licenza, elenco delle modifiche locali |
| [references/adattamento-tesi-giuridica.md](references/adattamento-tesi-giuridica.md) | raccordo con `tesi-contabilita-pubblica`: ondate tradotte in fasi F0–F7, limiti di `citation_checker.py` sulle fonti giuridiche italiane, stile |
| [templates/tesi-giuridica-italiana/](templates/tesi-giuridica-italiana/) | template LaTeX della tesi (italiano, capitoli romani, note a piè di pagina, bibliografia per sezioni) |
| [scripts/audit_note_tesi.py](scripts/audit_note_tesi.py) | audit di note, schede e registro delle fonti sui capitoli in Markdown |
